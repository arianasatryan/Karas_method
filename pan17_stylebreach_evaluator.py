...
def computeMeasures(inputText, groundTruthData, producedData):
    ...
    (groundTruthWordBorders, producedDataWordBorders, totalWordCount) = getWordPositionsFromCharacterPositions(inputText, groundTruthData["positions"], producedData["positions"])
...
def main(inputDataset, inputRun, outputDir):
    ...
    for file in os.listdir(inputDataset):
        if file.endswith(".txt"):
            ...
            inputText = ""
            with open(txtFileName,errors='ignore') as txtFile:
                ...
                if os.path.exists(producedTruthFilename):
                    with open(producedTruthFilename) as producedTruthFile:
                        ...
                        if not "positions" in truthData:
                            sys.exit("There is no 'positions' key in " + truthFileName)
                        if not "positions" in producedData:
                            sys.exit("There is no 'positions' key in " + producedTruthFilename)
